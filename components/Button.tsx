...
const Button: React.FC<ButtonProps> = ({ children, variant = 'primary', onClick, disabled, className }) => {
  const buttonClasses = `
    rounded-full h-fit
    ${variant === 'primary' ? 'py-2 px-4 w-48 text-white bg-[#0b754e] active:bg-[#1FAB4F]' : ''}
    ...
    ${variant === 'fixed-circle' ? 'fixed bottom-5 right-5 flex flex-row items-center rounded-full shadow-lg bg-[#0b754e] p-5 ml-auto' : ''}
