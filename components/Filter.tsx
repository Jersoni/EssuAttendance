...
const Filter = ({
  ...
  function handleCoursesChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (value === "AllCourses") {
      // if target is "All", set to All only
  ...
  useEffect(() => {
    ...
    if (courses.length === 0) setCourses(["AllCourses"]);
  }, [courses]);
  ...
  useEffect(() => {
    ...
    if (years.length === 0 || years.length === 4) setYears(["AllYears"]);
  }, [years]);
  ...
  return (
    <div>
      ...
      <div className="">
        <div
          className={`bg-gray-50 fixed w-[0vw] h-full right-0 mx-auto duration-300 ease-out transition-all bottom-0 overflow-hidden ${
            isOpen ? "!w-[85vw] xs:!w-[75vw]" : ""
          } z-[1500] flex flex-col text-sm border-l border-gray-300`}
        >
          <div className="text-lg bg-white border- p-3 pl-5 border-gray-200 flex flex-row items-center">
            <h2 className=" font-bold text-black text-opacity-80 w-full">
              Filters
            </h2>
          </div>
          ...
          <div className="flex flex-row gap-3 left-0 items-center justify-end p-3 pr-5 pb-8 border- border-gray-200 bg-white z-[600] mt-auto">
            <Button
              className=" min-w-24 !bg-gray-200 !border-0 !font-medium text-gray-600"
              variant="secondary"
        ...
        <div
          onClick={setIsOpen}
          className={`bg-black bg-opacity-20 fixed pointer-events-auto z-[1400] top-0 left-0 right-0 bottom-0 transition-all ${
            isOpen ? "block" : "hidden"
          }`}
        ></div>
      </div>
...
const DropDownChecklist = ({
  ...
  return (
    ...
    >
      ...
      {/* MODAL */}
      <div
        className={`w-full h-fit pb-3 border- border-gray-200 overflow-hidden
                ${isOpen && label === "COURSE" ? "" : ""}
                ${
        ...
        <div className="flex flex-row gap-2 flex-wrap h-fit">
          {options.map((option) => (
            ...
            >
              ...
              />
              <label htmlFor={option.value} className="w-full text-xs">
                {option.label}
              </label>
            </div>
          ))}
      ...
      {options.map((option, index) => (
        <div className="flex items-center relative py-1" key={option.value}>
          ...
          <label
            ...
            className="opacity-90 pl-2 text-sm z-[130]"
          >
            {option.label}
          </label>
        </div>
